...
class BGLLogsDescriptionPreprocessor(Preprocessor):
    def __init__(
        self, config: BGLPreprocessorConfig,
    ):
        self.config = config
    ...
    def _load_column_descriptions(
        self, bgl_df: pd.DataFrame, relevant_columns: Set[str]
    ) -> pd.DataFrame:
        column_descriptions = self._get_column_descriptions()
        description_records = []
        for column in relevant_columns:
            values = set(
                bgl_df[column].dropna().astype(str).replace(np.nan, "", regex=True)
            )
...
class BGLLogsHierarchyPreprocessor(Preprocessor):
    ...
    def _load_log_only_hierarchy(self, max_data_size=-1) -> pd.DataFrame:
        ...
        attribute_hierarchy = self._load_attribute_hierarchy(
            bgl_df, set(["coarse_log_cluster_path"])
        )
        return (
            pd.concat(
            [attribute_hierarchy, self._load_log_hierarchy(bgl_df, relevant_log_columns)], 
            ignore_index=True)
            .drop_duplicates()
            .reset_index(drop=True)
        )
    ...
    def _load_attribute_only_hierarchy(self, max_data_size=-1) -> pd.DataFrame:
        ...
        attribute_hierarchy = self._load_attribute_hierarchy(
            bgl_df, relevant_columns
        )
        return (
            pd.concat([attribute_hierarchy, self._load_log_hierarchy(bgl_df, relevant_columns)], ignore_index=True)
            .drop_duplicates()
            .reset_index(drop=True)
        )
...
class BGLLogsCausalityPreprocessor(Preprocessor):
    def __init__(
        self, config: BGLPreprocessorConfig,
    ):
        self.config = config
    ...
    def load_data(self, algorithm = "heuristic", max_data_size=-1) -> pd.DataFrame:
        ...
        if algorithm == "heuristic":
            ...
            transformed_df, evmap = transformer.transform_time_series_to_events(bgl_df, relevant_columns)
    ...
    def _generate_counted_causality(
        ...
        for _, row in tqdm(
            df.iterrows(),
            desc="Generating counted causality for BGL log data",
            total=len(df),
        ):
