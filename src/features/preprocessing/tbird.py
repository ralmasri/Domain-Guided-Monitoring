...
class ThunderBirdPreprocessorConfig:
    aggregated_log_file: Path = Path("data/logs_ThunderBird.csv")
    final_log_file: Path = Path("data/tbird.pkl")
...
class ThunderBirdLogsPreprocessor(Preprocessor):
    sequence_column_name: str = "all_events"

    def __init__(self, config: ThunderBirdPreprocessorConfig):
        self.config = config
    ...
    def _aggregate_per(
        ...
        logging.debug("Aggregating ThunderBird data per %s", aggregation_column)
...
class ThunderBirdLogsDescriptionPreprocessor(Preprocessor):
    def __init__(
        self, config: ThunderBirdPreprocessorConfig,
    ):
        self.config = config

    def load_data(self, max_data_size=-1) -> pd.DataFrame:
        preprocessor = ThunderBirdLogsPreprocessor(self.config)
        tbird_df = preprocessor._load_log_only_data(max_data_size)
        return self._load_column_descriptions(tbird_df, preprocessor.relevant_columns)

    def _load_column_descriptions(
        self, tbird_df: pd.DataFrame, relevant_columns: Set[str]
    ) -> pd.DataFrame:
        column_descriptions = self._get_column_descriptions()
        description_records = []
        for column in relevant_columns:
            values = set(
                tbird_df[column].dropna().astype(str).replace(np.nan, "", regex=True)
            )
...
class ThunderBirdLogsHierarchyPreprocessor(Preprocessor):
    def __init__(
        self, config: ThunderBirdPreprocessorConfig,
    ):
        self.config = config
    ...
    def _load_log_only_hierarchy(self, max_data_size=-1) -> pd.DataFrame:
        ...
        attribute_hierarchy = self._load_attribute_hierarchy(
            tbird_df, set(["coarse_log_cluster_path"])
        )
        return (
            pd.concat(
            [attribute_hierarchy, self._load_log_hierarchy(tbird_df, relevant_log_columns)], 
            ignore_index=True)
            .drop_duplicates()
            .reset_index(drop=True)
        )
    ...
    def _load_attribute_only_hierarchy(self, max_data_size=-1) -> pd.DataFrame:
        ...
        attribute_hierarchy = self._load_attribute_hierarchy(
            tbird_df, relevant_columns
        )
        return (
            pd.concat([attribute_hierarchy, self._load_log_hierarchy(tbird_df, relevant_columns)], ignore_index=True)
            .drop_duplicates()
            .reset_index(drop=True)
        )
    ...
    def _load_log_hierarchy(
        ...
        for _, row in tqdm(
            tbird_df.iterrows(),
            desc="Adding ThunderBird log hierarchy",
            total=len(tbird_df),
        ):
...
class ThunderBirdLogsCausalityPreprocessor(Preprocessor):
    def __init__(
        self, config: ThunderBirdPreprocessorConfig,
    ):
        self.config = config
    ...
    def load_data(self, algorithm = "heuristic", max_data_size=-1) -> pd.DataFrame:
        preprocessor = ThunderBirdLogsPreprocessor(self.config)
        tbird_df = preprocessor._load_log_only_data(max_data_size).fillna("")
        ...
        if algorithm == "heuristic":
            ...
            transformed_df, evmap = transformer.transform_time_series_to_events(tbird_df, relevant_columns)
    ...
    def _generate_counted_causality(
        ...
        for _, row in tqdm(
            df.iterrows(),
            desc="Generating counted causality for ThunderBird log data",
            total=len(df),
        ):
